...
// printing stdout to stdout if drake was run with -v.  It adds adds env to the
...
// Exec executes the command, piping its stderr to drake's stderr and
// piping its stdout to the given writer. If the command fails, it will return
// an error that, if returned from a target or dk.Deps call, will cause drake to
...
func Exec(env map[string]string, stdout, stderr io.Writer, cmd string, args ...string) (ran bool, err error) {
	...
	if ran {
		return ran, dk.Fatalf(code, `running "%s %s" failed with exit code %d`, cmd, strings.Join(args, " "), code)
	}
