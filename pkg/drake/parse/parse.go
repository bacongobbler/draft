...
import (
	...
	mgTypes "github.com/Azure/draft/pkg/drake/types"
)
...
// Function represented a job function from a drake file
...
func setDefault(p *doc.Package, pi *PkgInfo, info types.Info) {
	for _, v := range p.Vars {
		for x, name := range v.Names {
			...
			log.Println("warning: default declaration does not reference a drake target")
		}
	}
}
	...
	for _, v := range p.Vars {
		for x, name := range v.Names {
			...
			for _, elem := range comp.Elts {
				...
				if !valid {
					log.Printf("warning: alias declaration (%s) does not reference a drake target", alias)
				}
